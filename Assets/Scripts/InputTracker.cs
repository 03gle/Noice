        ...
        if (Physics.Raycast(leftController.position, leftController.forward, out hit, 100, layerMask))
        {
            ...
            for (int i = 0; i < trackingPointsL.Count; i++)
            {
                if (i < pointInfluenceLimit)
                {
                    ...
                    var res = eventInstanceL.setParameterByName(trackingPointsL[i].parameterName, Blend(trackingPointsL, i), true);
                    //Debug.Log(res);
                }
                else
                {
                    //Debug.Log("Left Hand " + i + ": 0");
                    eventInstanceL.setParameterByName(trackingPointsL[i].parameterName, 0, true);
                }
            }
